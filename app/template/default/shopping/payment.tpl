	...
	<form name="form1" id="form1" method="post" action="?">
				...
				<h2>ポイント使用の指定</h2>
					...
					<div class="point_announce">
						...
						<ul>
							...
							今回、<input type="text" name="<!--{$key}-->" value="<!--{$arrForm[$key].value|default:$tpl_user_point}-->" maxlength="<!--{$arrForm[$key].length}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" class="box60" />&nbsp;Ptを使用する。<span class="attention"><!--{$arrErr[$key]}--></span>
							</li>
		...
		<div class="pay_area02">
			...
			<div>
				...
				<span class="attention"><!--{$arrErr[$key]}--></span>
				<textarea name="<!--{$key}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" cols="70" rows="8" class="txtarea w95p" wrap="hard"><!--{"\n"}--><!--{$arrForm[$key].value|h}--></textarea>
		...
		<div class="btn_area">
			<ul>
				<li>
					<a href="?mode=return" class="btn btn-default" name="back03" id="back03">戻る</a>
				</li>
				<li>
					<input type="submit" class="btn btn-success" value="次へ" name="next" id="next" />
				</li>
			</ul>
		</div>
	</form>
