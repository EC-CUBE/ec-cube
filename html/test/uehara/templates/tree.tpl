...
<form name="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->">
	...
	<option value="<!--{$arrFileList[cnt].file_path}-->" <!--{if $arrFileList[cnt].file_path eq $arrParam['select_file']}-->selected<!--{/if}-->><!--{$arrFileList[cnt].file_name}-->　<!--{$arrFileList[cnt].file_size}-->　<!--{$arrFileList[cnt].file_time}--></option>
	<!--{/section}-->
	</select><br/>
