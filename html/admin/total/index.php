...
function lfGetOrderMember($type, $sdate, $edate, $objPage, $graph = true) {
	...
	if($graph) {	
		$image_key = "member";
		$objPage->tpl_image = lfGetGraphPie($objPage->arrResults, "member_name", $image_key, "(売上比率)");
	}
...
function lfGetOrderProducts($type, $sdate, $edate, $objPage, $graph = true) {
	...
	if($graph) {
		...
		$objPage->tpl_image = lfGetGraphPie($objPage->arrResults, "name", $image_key, "(売上比率)");
	}
