...
function lfProductClassError($array) {
	...
	while($array["classcategory_id1:".$no] != "") {
		if($array["check:".$no] == 1) {
			...
			$objErr->doFunc(array("参考市場価格", "price01:".$no, PRICE_LEN), array("ZERO_CHECK", "NUM_CHECK", "MAX_LENGTH_CHECK"));
			$objErr->doFunc(array("価格", "price02:".$no, PRICE_LEN), array("EXIST_CHECK", "ZERO_CHECK", "NUM_CHECK", "MAX_LENGTH_CHECK"));
