...
<tr>
	...
	<table width="500" border="0" cellspacing="1" cellpadding="8" summary=" ">
		...
		<tr>
			...
			<span class="red"><!--{$arrErr[$key]}--></span>
			<input type="text" name="<!--{$key}-->" value="<!--{$arrForm[$key].value|escape}-->" maxlength="<!--{$arrForm[$key].length}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" <!--{if $arrForm.db_type.value eq 'pgsql'}-->disabled=true bgcolor="#00ff00"<!--{else}-->disabled=false bgcolor="#ff00ff"<!--{/if}--> size="6" class="box6" />
			</td>
		</tr>
		<tr>
