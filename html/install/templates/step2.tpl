...
<tr>
	...
	<table width="500" border="0" cellspacing="1" cellpadding="8" summary=" ">
		...
		<tr>
			...
			<span class="red"><!--{$arrErr[$key]}--></span><!--{$arrForm.db_type.value}-->
			<input type="text" name="<!--{$key}-->" value="<!--{$arrForm[$key].value|escape}-->" maxlength="<!--{$arrForm[$key].length}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" <!--{if $arrForm.db_type.value == 'mysql'}-->disabled=false<!--{else}-->disabled=false;<!--{/if}--> size="6" class="box6" />
			</td>
		</tr>
		<tr>
