...
@Features("商品を購入する")
public class PurchaseProductWithBeforeMethodTest extends BaseTest {
    private UserForExercise user;

    @DataProvider(name = "product")
    public static Object[][] parameters() {
        String path = "/fixtures/eccube3/scenarios/exercise/PurchaseProductTest/product.yml";
        return new Object[][]{
                { Product.fromYaml(path).descriptionDetail("パターン1"), "パターン1" },
                { Product.fromYaml(path).descriptionDetail("パターン2"), "パターン2" },
                { Product.fromYaml(path).descriptionDetail("パターン3"), "パターン3" },
                { Product.fromYaml(path).descriptionDetail("パターン4"), "パターン4" },
                { Product.fromYaml(path).descriptionDetail("パターン5"), "パターン5" },
        };
    }

    // setUpにも引数を付けることでDataProviderで作ったデータを引き継げる
    @BeforeMethod
    public void setUp(Object[] testArgs) {
        ...
        productNewPage.registerProduct((Product) testArgs[0]);
    }

    @Stories("商品を購入できる")
    @Test(dataProvider = "product")
    public void AuthenticatedUserCanPurchaseProduct(Product product, String message) throws Exception {
        // 登録した商品の詳細画面を開く
