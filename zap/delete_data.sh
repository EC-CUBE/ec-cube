...
delete from dtb_block_position where layout_id in (select id from dtb_layout where create_date between now() - interval '24:00' and now() - interval '00:20:00');
delete from dtb_block where create_date between now() - interval '24:00' and now() - interval '00:20:00' and id not in (select distinct block_id from dtb_block_position);
