...
delete from dtb_delivery where create_date between now() - interval '24:00' and now() - interval '01:00:00';
delete from dtb_payment where create_date between now() - interval '24:00' and now() - interval '00:00:06';
delete from dtb_block_position where layout_id in (select id from dtb_layout where create_date between now() - interval '24:00' and now() - interval '00:10:00');
delete from dtb_block where create_date between now() - interval '24:00' and now() - interval '00:10:00' and id not in (select distinct block_id from dtb_block_position);
delete from dtb_layout where create_date between now() - interval '24:00' and now() - interval '01:00:00';
