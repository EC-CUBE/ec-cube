...
class LoggerRendererMap {
     ...
     */
    function &getByClassName($clazz)
    {
        $r = null;
        for($c = strtolower($clazz); !empty($c); $c = get_parent_class($c)) {
            if (isset($this->map[$c])) {
                return  $this->map[$c];
            }
        }
     ...
     */
    function put($clazz, $or)
    {
        $this->map[strtolower($clazz)] = $or;
    }
     ...
     * @return boolean
     */
    function rendererExists($clazz)
    {
        $clazz = basename($clazz);
        @include_once(LOG4PHP_DIR ."/or/{$clazz}.php");
