...
class Initial
{
    ...
    {
        define('CONFIG_REALFILE', realpath(dirname(dirname(__FILE__))) . '/../config/config.php');
    ...
    public function normalizeHostname()
    {
        ...
        $netUrlCorrect = new \Net_URL(\Eccube\Common\Util\Utils::sfIsHTTPS() ? HTTPS_URL : HTTP_URL);
        // 設定上のホスト名
        ...
        if ($request_hostname !== $correct_hostname) {
            ...
            // リダイレクト(恒久的)
            \Eccube\Common\Response::sendHttpStatus(301);
            \Eccube\Common\Response::sendRedirect($correct_url);
        }
    }
}
