...
namespace Eccube\Common\Helper;
 ...
 * 現状 Eccube\Common\Util\GcUtils に依存しているため、その中で構文エラーは捕捉できない。
 * @package Helper
 */
class HandleErrorHelper
{
    ...
    public static function displaySystemError($errstr = null)
    {
        ...
        foreach ($arrObs as $arrOb) {
            if ($arrOb['name'] === 'Eccube\\Common\\MobileEmoji::handler') {
        ...
        if (!$loaded_ob_emoji && class_exists('Eccube\\Common\\MobileEmoji')) {
            ob_start(array('Eccube\\Common\\MobileEmoji', 'handler'));
        }
