...
class SC_FormParam {
    ...
    function setHtmlDispNameArray() {
        foreach ($this->keyname as $index => $key) {
            ...
            if ($find) {
                $this->html_disp_name[$index] = t('SC_FormParam_003', array('T_ARG1' => $this->disp_name[$index]));
            } else {
                $this->html_disp_name[$index] = $this->disp_name[$index];
            }
            if ($this->arrDefault[$key] != '') {
                $this->html_disp_name[$index] .= t('SC_FormParam_004', array('T_ARG1' => $this->arrDefault[$key]));
            }
                ...
                switch ($func) {
                    ...
                    case 'FILE_EXISTS':
                        if ($value != '' && !file_exists($this->check_dir . $value)) {
                            $arrErr[$key] = t('SC_FormParam_001', array('T_ARG1' => $this->disp_name[$index]));
                        }
                        break;
                    // ダウンロード用ファイルの存在チェック
                    case 'DOWN_FILE_EXISTS':
                        ...
                        break;
                    default:
                        $arrErr[$key] = t('SC_FormParam_002', array('T_ARG1' => $func));
                        break;
                }
            }
