...
class SC_CustomerList extends SC_SelectSql {
	...
	function SC_CustomerList($array, $mode = '') {
        ...
        if ( $mode == 'magazine' ){
        	...
        	if($val==2){
        		if($this->arrSql['mail_type']==1){
            	$this->setWhere( "email NOT ILIKE '%@docomo.ne.jp%' OR email NOT LIKE '%@softbank.ne.jp%'" );
        	}elseif($this->arrSql['mail_type']==2){
        		$this->setWhere( "email_mobile NOT ILIKE '%@docomo.ne.jp%' OR email_mobile NOT LIKE '%@softbank.ne.jp%'" );
        	    }
        	}
        }
