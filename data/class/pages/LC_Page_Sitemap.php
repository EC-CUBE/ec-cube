...
class LC_Page_Sitemap extends LC_Page {
    ...
    function getAllDetail() {
        ...
        foreach ($result as $row) {
            ...
            $page["url"] = HTTP_URL . substr(P_DETAIL_URL_PATH, strlen(URL_PATH)) . $row['product_id'];
            $arrRet[] = $page;
            
            // モバイルサイト
            if (USE_MOBILE !== false) {
                $page["url"] = HTTP_URL . substr(MOBILE_P_DETAIL_URL_PATH, strlen(URL_PATH)) . $row['product_id'];
                $arrRet[] = $page;
            }
        }
        return $arrRet;
