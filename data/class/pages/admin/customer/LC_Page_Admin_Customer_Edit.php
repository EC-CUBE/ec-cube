    ...
    function lfCheckError(&$objFormParam) {
        ...
        if (!SC_Utils_Ex::isBlank($arrData['email'])) {
            if ($arrData['email'] == $objFormParam->getValue('email')) {
                $arrErr['email'] = t('LC_Page_Admin_Customer_Edit_003', array('%s1' => $arrData['customer_id']));
            } else if ($arrData['email'] == $objFormParam->getValue('email_mobile')) {
                $arrErr['email_mobile'] = t('LC_Page_Admin_Customer_Edit_003', array('%s1' => $arrData['customer_id']));
            }
        }
        if (!SC_Utils_Ex::isBlank($arrData['email_mobile'])) {
            if ($arrData['email_mobile'] == $objFormParam->getValue('email_mobile')) {
                $arrErr['email_mobile'] = t('LC_Page_Admin_Customer_Edit_004', array('%s1' => $arrData['customer_id']));
            } else if ($arrData['email_mobile'] == $objFormParam->getValue('email')) {
    if ($arrErr['email'] == '') {
                    $arrErr['email'] = t('LC_Page_Admin_Customer_Edit_004', array('%s1' => $arrData['customer_id']));
                }
            }
        }
        return $arrErr;
