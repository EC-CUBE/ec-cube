...
class SC_UploadFile {
    ...
    function makeTempFile($keyname, $rename = IMAGE_RENAME) {
        ...
        if (!($_FILES[$keyname]['size'] > 0)) {
            $objErr->arrErr[$keyname] = SC_I18n_Ex::t('SC_UploadFile_001', array('T_DISPNAME' => $this->disp_name[$arrKeyname[$keyname]]));
        } else {
                    ...
                    if (!isset($objErr->arrErr[$keyname])) {
                        ...
                        // 画像ファイル以外の場合
                            ...
                            if (move_uploaded_file($_FILES[$keyname]['tmp_name'], $this->temp_dir . $this->temp_file[$cnt])) {
                                ...
                                $objErr->arrErr[$keyname] = SC_I18n_Ex::t('SC_UploadFile_003');
    ...
    function makeTempDownFile($keyname='down_file') {
        ...
        $arrKeyname = array_flip($this->keyname);
        if (!($_FILES[$keyname]['size'] > 0)) {
            $objErr->arrErr[$keyname] = SC_I18n_Ex::t('SC_UploadFile_002', array('T_DISPNAME' => $this->disp_name[$arrKeyname[$keyname]]));
        } else {
            foreach ($this->keyname as $val) {
                // 一致したキーのファイルに情報を保存する。
                if ($val == $keyname) {
            ...
            if ($val == $keyname || $keyname == '') {
                ...
                if ($this->necessary[$cnt] == true) {
                    ...
                    if ($this->save_file[$cnt] == ''
                        ...
                        $arrRet[$val] = SC_I18n_Ex::t('SC_UploadFile_001', array('T_DISPNAME' => $this->disp_name[$cnt]));
                    }
                }
            }
            $cnt++;
