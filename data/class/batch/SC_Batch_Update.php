...
class SC_Batch_Update extends SC_Batch {
    ...
    function execute($target = '.') {
        ...
        if (!is_writable($bkupPath) || !is_writable($bkupPathFile)) {
            $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_FAIL_MAKEBACKUPDIR');
            $arrLog['err'][] = $msg;
            $this->printLog($msg);
            return $arrLog;
        }
        ...
        foreach ($fileArrays as $path) {
            if (is_file($path)) {
                ...
                if (in_array($fileName, $excludeArray)) {
                    ...
                    $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_EXCLUDE_FILE') . $path;
                    $this->printLog($msg);
                    continue;
                }
                ...
                foreach ($includeArray as $include) {
                    if ($suffix == $include) {
                        ...
                        if (!empty($distinfo[$sha1])) {
                            ...
                            $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_HASH_UNMATCH') . $path;
                            $arrLog['err'][] = $msg;
                            $this->printLog($msg);
                            break 2;
                        }
                        ...
                        if (file_exists($out) && $sha1 == sha1_file($out)) {
                            $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_SKIP_SAME') . $out;
                            $this->printLog($msg);
                            continue;
                        }
                        ...
                        if (file_exists($out)) {
                            ...
                            if (!@copy($out, $bkupTo)) {
                                $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_FAIL_MAKEBACKUPFILE') . $out . ' -> ' . $bkupTo;
                                $arrLog['err'][] = $msg;
                                $this->printLog($msg);
                                break 2;
                            }
                        ...
                        if (!$handle) {
                            ...
                            $handle = @fopen($out, 'w');
                            if (!$handle) {
                                $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_UNWRITABLE') . $out;
                                $arrLog['err'][] = $msg;
                                $this->printLog($msg);
                                continue;
                            }
                        ...
                        $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_SUCCESS_FILECOPY') . $out;
                        $arrLog['ok'][] = $msg;
                        $this->printLog($msg);
                        // ファイルを閉じる
                        fclose($handle);
        ...
        if (is_writable($bkupPath)) {
            ...
            @fwrite($handle, $src);
            @fclose($handle);
            $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_SUCCESS_MAKEDISTINFO') . $bkupPath . 'distinfo.php';
            $this->printLog($msg);
        } else {
            $msg = SC_I18n_Ex::t('SC_BATCH_UPDATE_FAIL_MAKEDISTINFO') . $bkupPath . 'distinfo.php';
            $arrLog['err'][] = $msg;
            $this->printLog($msg);
        }
        umask($oldMask);
