			...
			<tr>
				...
				<table border="0" cellspacing="0" cellpadding="0" summary=" ">
					<tr>
						...
						<td class=<!--{if $arrPagenavi.now_page == $item}-->"number-on"<!--{else}-->"number"<!--{/if}-->><a href="<!--{$smarty.server.PHP_SELF}-->" onclick="fnNaviSearchPage(<!--{$item}-->, '<!--{$arrPagenavi.mode}-->'); return false;" onmouseover="this.style.backgroundColor = '#f7c600'; this.style.background = 'url(http://test.ec-cube.net/img/contents/number_bg_on.jpg)'" <!--{if $arrPagenavi.now_page == $item}-->onmouseout="this.style.backgroundColor = '#505468'; this.style.background = 'url(http://test.ec-cube.net/img/contents/number_bg.jpg)'<!--{/if-->"><!--{$item}--></a></td>
						<td><img src="/img/contents/number_line.jpg" width="2" height="17" alt=""></td>
						<!--{/foreach}-->
					</tr>
				</table>
