			...
			<tr>
				...
				<table border="0" cellspacing="0" cellpadding="0" summary=" ">
					<tr>
						...
						<td class=<!--{if $arrPagenavi.now_page == $item}-->"number-on"<!--{else}-->"number"<!--{/if}-->><a href="<!--{$smarty.server.PHP_SELF}-->" onclick="fnNaviSearchPage(<!--{$item}-->, '<!--{$arrPagenavi.mode}-->'); return false;" onmouseover="naviStyleChange2('test<!--{$item}-->', '#f7c600', 'http://test.ec-cube.net/img/contents/number_bg_on.jpg')" <!--{if $arrPagenavi.now_page == $item}-->onmouseover="naviStyleChange2('test<!--{$item}-->', '#505468', 'http://test.ec-cube.net/img/contents/number_bg.jpg')"<!--{/if}-->id = 'test<!--{$item}-->'><!--{$item}--></a></td>
						<td><img src="/img/contents/number_line.jpg" width="2" height="17" alt=""></td>
						<!--{/foreach}-->
					</tr>
				</table>
