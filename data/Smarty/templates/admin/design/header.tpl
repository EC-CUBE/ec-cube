...
<div id="design" class="contents-main">
  ...
  <dic id="design-header-preview">
    <!--{if $browser_type == 1 }-->
      <div style="zoom:0.8"><!--{include file="`$smarty.const.USER_FILE_PATH`include/preview/header.tpl"}--></div>
    <!--{ else }-->
  ...
  <input type="hidden" name="device_type_id" value="<!--{$device_type_id|h}-->" />
    ...
    <div class="btn">
      ...
      <a class='button' href="javascript:;" name='preview' onclick="lfnSetBrowser('form_header', 'browser_type'); fnFormModeSubmit('form_header','preview','','');"><span>プレビュー</span></a>
    </div>
  </form>
  <!--{* ▲ヘッダー編集ここまで *}-->
  ...
  <div id="design-footer-preview">
    <!--{if $browser_type == 1 }-->
      <div style="zoom:0.8"><!--{include file="`$smarty.const.USER_FILE_PATH`/include/preview/footer.tpl"}--></div>
    <!--{ else }-->
      <span class="attention"><strong>プレビューはIEでのみ表示されます。</strong></span>
    <!--{ /if }-->
  </div>
  ...
  <input type="hidden" name="device_type_id" value="<!--{$device_type_id|h}-->" />
    ...
    <div class="btn">
      ...
      <a class='button' href="javascript:;" name='preview' onclick="lfnSetBrowser('form_footer', 'browser_type'); fnFormModeSubmit('form_footer','preview','','');"><span>プレビュー</span></a>
    </div>
  </form>
  <!--{* ▲フッター編集ここまで *}-->
