								...
								<table width="678" border="0" cellspacing="1" cellpadding="8" summary=" ">
									...
									<!--{*foreach key=key item=item from=$arrTemplate.image*}-->
										...
										<td bgcolor="#ffffff" align="center"><!--{$arrTemplate[template]}--><!--{$code}-->
											...
											<label for="radio<!--{$key}-->"><input type="radio" name="check_template" value=<!--{$key}--> id="radio<!--{$key}-->" onClick="ChangeImage('<!--{$image}-->');" <!--{if $arrTemplate.check[$key] != ""}-->checked<!--{/if}-->>選択</label>
										</td>
										<!--{assign var=cnt value=$smarty.section.template.iteration-1}-->
