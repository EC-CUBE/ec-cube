							...
							<tr class="fs12n">
								<td bgcolor="#ffffff" align="left" colspan=2>
									...
									URL：<!--{if $arrPageData.edit_flg == 2}-->
										...
										<!--{else}-->
											<!--{$user_URL}--><input type="text" name="url" value="<!--{$arrPageData.directory|escape}--><!--{$arrPageData.filename|escape}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" style="<!--{if $arrErr.url != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}--><!--{/if}-->" size="60" class="box60" />.php<span class="red"> （上限<!--{$smarty.const.STEXT_LEN}-->文字）</span>
										<!--{/if}-->
								</td>
							</tr>
							<tr class="fs12n">
