...
function lfnDispChange(){
	...
	if(document.getElementById('disp_url1').style.display == 'none'){
		for (i = 1; i <= cnt; i++) {
			...
			document.getElementById(inner_id).innerHTML = '	URL <FONT Color="yellow" Size=3><a href="#" onClick="lfnDispChange();"> >> カテゴリ表示</a></FONT>';
		}
	}else{
		for (i = 1; i <= cnt; i++) {
			disp_id = 'disp_url'+i;
				...
				<tr>
					...
					<table width="840" border="0" cellspacing="1" cellpadding="5" summary=" ">
						...
						<tr bgcolor="<!--{$arrPRODUCTSTATUS_COLOR[$status]}-->" class="fs12n">
							...
							<td align="center"><span class="icon_edit"><a href="<!--{$smarty.const.URL_DIR}-->" onclick="fnChangeAction('./product.php'); fnModeSubmit('copy', 'product_id', <!--{$arrProducts[cnt].product_id}-->); return false;" >複製</a></span></td>
						</tr>
						<!--▲商品<!--{$smarty.section.cnt.iteration}-->-->
						<!--{/section}-->
