
<!--¢§CONTENTS-->
<table width="780" border="0" cellspacing="0" cellpadding="0" summary=" ">
	<tr>
		<td bgcolor="#ffffff" align="center" valign="top" height="400">
		<table width="740" border="0" cellspacing="0" cellpadding="0" summary=" ">
			<tr>
				<!--¢§SUB NAVI-->
				<td class="fs12n"><!--{include file=$tpl_subnavi}--></td>
				<!--¢¥SUB NAVI-->
			</tr><tr><td height="25"></td></tr>
		</table>
		
		<!--¢§MAIN CONTENTS-->
		<table width="740" border="0" cellspacing="0" cellpadding="0" summary=" ">
			<tr>
				<td class="fs14n"><strong>¢£¥ì¥Ó¥å¡¼ÅÐÏ¿</strong></td>
			</tr>
			<tr><td height="10"></td></tr>
		</table>
		
		<!--¢§ÅÐÏ¿¥Æ¡¼¥Ö¥ë¤³¤³¤«¤é-->
		<form name="form1" id="form1" method="post" action="">
		<table width="740" border="0" cellspacing="1" cellpadding="5" summary=" " bgcolor="#cccccc">
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">Åê¹ÆÆü</td>
				<td bgcolor="#ffffff" width="607">
				<select name="year_from">
				<option value="" selected="selected">------</option>
				</select>Ç¯
				<select name="month_from">
				<option value="" selected="selected">----</option>
				</select>·î
				<select name="day_from">
				<option value="" selected="selected">----</option>
				</select>Æü</td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">Åê¹Æ¼ÔÌ¾</td>
				<td bgcolor="#ffffff" width="607"><input type="text" name="name" size="30" class="box30" /></td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">À­ÊÌ</td>
				<td bgcolor="#ffffff" width="607"><input type="checkbox" name="sex01" value="ÃËÀ­" />ÃËÀ­¡¡<input type="checkbox" name="sex02" value="½÷À­" />½÷À­</td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">¥á¡¼¥ë¥¢¥É¥ì¥¹</td>
				<td bgcolor="#ffffff" width="607"><input type="text" name="mail" size="30" class="box30" /></td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">¾¦ÉÊÌ¾</td>
				<td bgcolor="#ffffff" width="607"><input type="text" name="product_mname" size="30" class="box30" /></td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">¤ª¤¹¤¹¤á¥ì¥Ù¥ë</td>
				<td bgcolor="#ffffff" width="607"><select name="recomend">
				<option value="" selected="selected">ÁªÂò¤·¤Æ¤¯¤À¤µ¤¤</option>
				<option value="¡ú¡ú¡ú¡ú¡ú">¡ú¡ú¡ú¡ú¡ú</option>
				<option value="¡ú¡ú¡ú¡ú">¡ú¡ú¡ú¡ú</option>
				<option value="¡ú¡ú¡ú">¡ú¡ú¡ú</option>
				<option value="¡ú¡ú">¡ú¡ú</option>
				<option value="¡ú">¡ú</option>
				</select></td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">¥¿¥¤¥È¥ë</td>
				<td bgcolor="#ffffff" width="607"><input type="text" name="title" size="30" class="box30" /></td>
			</tr>
			<tr class="fs12n">
				<td bgcolor="#f0f0f0" width="110">¥³¥á¥ó¥È</td>
				<td bgcolor="#ffffff" width="607"><textarea name="textfield01" cols="60" rows="8" class="area60"></textarea></td>
			</tr>
		</table>
		<!--¢¥ÅÐÏ¿¥Æ¡¼¥Ö¥ë¤³¤³¤Þ¤Ç-->
		
		<br />
		<input type="button" name="subm" value="¤³¤ÎÆâÍÆ¤ÇÅÐÏ¿¤¹¤ë" />
		</form>
		
		<!--¢¥MAIN CONTENTS-->
		</td>
	</tr>
</table>
<!--¢¥CONTENTS-->
