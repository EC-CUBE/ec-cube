...
if(!empty($_GET["clientip"])){
    $objQuery->begin();
    $order_id = lfDoComplete($objQuery, $arrResult, $log_path);
    $objQuery->commit();
}

// 完了メール送信
if(sfIsInt($order_id) && $order_id > 0) {
...
//---------------------------------------------------------------------------------------------------------------------------------
// 完了処理
function lfDoComplete($objQuery, $arrResult, $log_path) {
    global $objCartSess;
    global $objSiteSess;
    $objCampaignSess = new SC_CampaignSession();
    $objCustomer = new SC_Customer();
    ...
    gfprintlog($objCustomer->isLoginSuccess(), $log_path);
    ...
    $arrCrilentIP = $objQuery->select("memo02, memo05", "dtb_payment", "module_id = ? and del_flg = 0", array(MDL_ZERO_ID));
    
    // 加盟店コードが違う場合にはエラー
    if(!(count($arrResult) > 0)){
        gfPrintLog("決済サーバからデータを正しく受け取れませんでした。", $log_path);
        return;
    }
    
    if(!in_array($arrResult["clientip"], $arrCrilentIP[0])) {
        gfPrintLog("加盟店コードが違います。", $log_path);
        return;
    }
    ...
    if(count($arrData) <= 0) {
        gfPrintLog("受注一時テーブルに指定したIDのデータがありません。", $log_path);
        return;
    }
    
    // 決済完了済みであればエラー
    if($arrData["del_flg"] == 1) {
        gfPrintLog("指定したIDのデータは決済完了済みです。", $log_path);      
        return;
    }
    
    // 一時受注テーブルのお支払い合計と、ゼロから返ってきた金額とが違う場合はエラー
    if($arrData["payment_total"] != $money) {
        gfPrintLog("お支払い金額が一致しません。", $log_path);
        return;
    }
    ...
    // 応答結果が正常でない場合
    if($arrResult["result"] != 'ok' && $arrResult["result"] != 'OK') {
        gfPrintLog("応答結果NG", $log_path);
        return;
    }
