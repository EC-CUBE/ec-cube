---
name: eccube-review-responsibility
...
2. [`eccube-pre-impl`](../eccube-pre-impl/SKILL.md) の各レイヤ節（全レイヤの「よくある間違い」を集約）を
   **正典として読み込み**、差分を照合する（詳細は再記述せず、そちらを参照する）。
3. 層をまたぐ観点（下記「層境界」）を最後に確認する。
4. 指摘は「**新規・改修分**」を優先。具体的な是正案（どの処理をどこへ出すか）を添える。

## 層ごとの観点（`eccube-pre-impl` の該当レイヤ節を参照）

### 責務分離（コントローラ / サービス） — Skill [`eccube-controller`](../eccube-controller/SKILL.md) / [`eccube-service`](../eccube-service/SKILL.md)
...
### セキュリティ — Skill [`eccube-security`](../eccube-security/SKILL.md)
...
### Twig 拡張・テンプレート — Skill [`eccube-twig-template`](../eccube-twig-template/SKILL.md)
...
### イベント — Skill [`eccube-event-subscriber`](../eccube-event-subscriber/SKILL.md)
- リスナー/サブスクライバに**業務ロジックが偏っていないか**（重い処理は Service へ委譲、リスナーは薄く）。
- `getSubscribedEvents()` が `static` か、イベント名に `EccubeEvents` 定数を使っているか。

### Entity — Skill [`eccube-entity`](../eccube-entity/SKILL.md)
...
### Repository — Skill [`eccube-repository`](../eccube-repository/SKILL.md)
- 生 SQL 連結でなく QueryBuilder＋`setParameter` か。画面表示の一覧・関連取得が無制限になっていないか（ページング/上限）。

### プラグイン — Skill [`eccube-plugin`](../eccube-plugin/SKILL.md)
