...
1. **まず置き場を決める** — 本体へのコントリビュート（Issue 対応・機能追加・バグ修正）なら `src/Eccube/`。
   `app/Customize/` は店舗のプロジェクト固有コード用で、本体の機能追加には使わない。
   判別できない依頼なら実装を始める前に確認する（**間違えると成果物がまるごと無駄になる**）
2. **下表から該当 Skill を特定し、すぐ Read する**（調査・grep より先）
3. 複数レイヤにまたがる場合は **該当 Skill をすべて** 読む
4. レイヤ規約は置き場（コア・`app/Customize`・プラグイン）に関わらず同じものが適用される
5. 設計案を出す前に、読んだ Skill の制約を設計に反映する
