...
- Twig 出力のエスケープ（XSS）は Skill `eccube-twig-template` を参照（`|raw` の濫用に注意）。
...
このレイヤの「よくある間違い」は [`eccube-pre-impl`](../eccube-pre-impl/SKILL.md) の「セキュリティ（認証・認可・CSRF・IDOR）」節に集約している。
...
実装・改修後は、Skill `eccube-review-responsibility` で責務分離とあわせて認可・CSRF・IDOR を点検すること。
