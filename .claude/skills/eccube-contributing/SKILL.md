...
- **新規 PHP ファイルにはライセンスヘッダ必須**（PHP-CS-Fixer の `header_comment` で強制。欠落は CI で落ちる）:
   ...
   */
  ```
- **型宣言を付ける**（PHPStan level 6 を通す）。レイヤ別の実装規約は各 Skill（`eccube-controller` / `eccube-service` / `eccube-entity` / `eccube-repository` / `eccube-formtype` / `eccube-migration` / `eccube-phpunit` …）に従う。
- **機能追加・修正にはテストを伴わせる**（PHPUnit。テストの書き方は Skill `eccube-phpunit`）。
...
5. 実装直後の自己レビュー（Skill `eccube-review-responsibility`）で責務分離・セキュリティ・レイヤ違反を点検。
