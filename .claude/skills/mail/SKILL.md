...
> Skill `service`（業務ロジックの置き場所）と対で使う。
...
購読してメールを差し替えるときは Skill `event-subscriber` を参照（`EventArgs` から `message` を取り出して件名・宛先・本文を変更）。
...
実装・改修後は、Skill `service`（業務ロジックの責務分離）・`event-subscriber`（差し替え）・
`twig-template`（テンプレートのエスケープ）と `review-responsibility` で点検すること。
