...
class PaginationTest extends EccubeTestCase
{
    ...
    public function testSortWithJoinPluginEntity()
    {
        ...
        $qb
            ...
            ->leftJoin(\Eccube\Tests\Doctrine\ORM\Tools\TestEntity::class, 'test', 'WITH', 'p.id = test.id')
            ->groupBy('p')
            ->addGroupBy('test')
            ->orderBy('col', 'DESC')
    ...
    public function testWhereWithSubQueryPluginEntity()
    {
        ...
        // テスト用のエンティティを検索するクエリ
        $repository = $this->entityManager->getRepository(\Eccube\Tests\Doctrine\ORM\Tools\TestEntity::class);
