...
class PL01RecommendCest
{
    ...
    public function recommend_02(AcceptanceTester $I)
    {
        ...
        $I->retrySeeInSource('おすすめ商品管理プラグイン', $recommendPluginRow);
        $I->retrySeeInSource('無効', $recommendPluginRow);
        $I->clickWithLeftButton("(//tr[contains(.,'おすすめ商品')]//i[@class='fa fa-play fa-lg text-secondary'])[1]");
        $I->retrySeeInSource('「おすすめ商品管理プラグイン」を有効にしました。');
        $I->retrySeeInSource('おすすめ商品管理プラグイン', $recommendPluginRow);
        $I->retrySeeInSource('有効', $recommendPluginRow);
        $I->clickWithLeftButton('(//li[@class="c-mainNavArea__navItem"])[5]');
        $I->wait(2);
        $I->retrySeeInSource('おすすめ管理', '(//li[@class="c-mainNavArea__navItem"])[5]');
    }
    ...
    public function recommend_05(AcceptanceTester $I)
    {
        ...
        $I->retrySeeInSource('オススメ', '.ec-layoutRole__contents');
        $I->amOnPage('/admin/content/layout/1/edit');
        ...
        $I->retrySeeInSource('オススメ', '.ec-layoutRole__contentBottom');
    }
    ...
    public function recommend_07(AcceptanceTester $I)
    {
        ...
        $I->retrySeeInSource('sand-01');
        $I->retrySeeInSource('説明分テスト');
        // フロント側チェック
        $I->amOnPage('/');
        $I->retrySeeInSource('チェリーアイスサンド', '.ec-layoutRole__mainBottom');
        $I->retrySeeInSource('説明分テスト', '.ec-layoutRole__mainBottom');
    }
    ...
    public function recommend_08(AcceptanceTester $I)
    {
        // フロント側チェック
        $I->amOnPage('/');
        $I->retrySeeInSource('チェリーアイスサンド', '(//li[@class="ec-shelfGrid__item"])[1]');
        $I->retrySeeInSource('彩のジェラートCUBE', '(//li[@class="ec-shelfGrid__item"])[2]');
        // おすすめ商品順番更新
        $I->amOnPage('/admin/plugin/recommend');
        $I->retrySeeInSource('チェリーアイスサンド');
        $I->retrySeeInSource('彩のジェラートCUBE');
        $I->dragAndDropByXPath('(//li[@class="list-group-item sortable-item ui-sortable-handle"])[1]', 174, 250, 5);
        $I->wait(5);
        // フロント側チェック
        $I->amOnPage('/');
        $I->retrySeeInSource('チェリーアイスサンド', '(//li[@class="ec-shelfGrid__item"])[2]');
        $I->retrySeeInSource('彩のジェラートCUBE', '(//li[@class="ec-shelfGrid__item"])[1]');
    }
    ...
    public function recommend_09(AcceptanceTester $I)
    {
        ...
        $I->wait(5);
        $I->retrySeeInSource('削除', '.modal-footer');
        $I->clickWithLeftButton('(//a[@class="btn btn-ec-delete"])[2]');
        $I->retrySeeInSource(' おすすめ商品を削除しました。');
        // フロント側チェック
        $I->amOnPage('/');
    ...
    public function recommend_10(AcceptanceTester $I)
    {
        ...
        $I->clickWithLeftButton('.btn.btn-ec-conversion.px-5.ladda-button');
        $I->retrySeeInSource('おすすめ商品を修正しました。');
        $I->retrySeeInSource('チェリーアイスサンド');
    ...
    public function recommend_12(AcceptanceTester $I)
    {
        ...
        $I->retrySeeInSource('インストール', $xpath);
    }
}
