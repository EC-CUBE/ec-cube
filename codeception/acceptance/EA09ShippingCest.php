...
class EA09ShippingCest
{
    ...
    public function shipping_出荷CSV登録(AcceptanceTester $I)
    {
        ...
        // 入金済みに更新しておく
        $Status = $entityManager->getRepository(\Eccube\Entity\Master\OrderStatus::class)->find(OrderStatus::PAID);
    ...
    public function shipping_出荷CSV登録失敗(AcceptanceTester $I)
    {
        ...
        $Orders = (Fixtures::get('createOrders'))($Customer, 3);
        // キャンセルに更新しておく
        $Status = $entityManager->getRepository(\Eccube\Entity\Master\OrderStatus::class)->find(OrderStatus::CANCEL);
        foreach ($Orders as $newOrder) {
            $newOrder->setOrderStatus($Status);
        }
        $entityManager->flush();
